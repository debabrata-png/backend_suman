...
exports.createphddeclare= async (req,res) => {
    //res.cookie("user","Akshata");
    to=req.query.to;
    contentr=req.query.contentr;
    fromemail=req.query.fromemail;
    password=req.query.password;
    subject='PhD declaration forms';

    colid=req.query.colid;

    phddir = './naac/' + colid.toString() + '/phddir';
    ...
    try{
        ...
        createphddeclaration();
        var dt1=new Date();

        const pat1= await pstatusnew.findOneAndUpdate({colid: colid, client: 'phddeclaration'},{
            remarks:'Mail sent to ' + to + ' on ' + dt1.toString(),
            colid:colid,
            name:"AI",
            user:"ai@campus.technology",
            client:'phddeclaration'
        }, {
            new: true,
            upsert: true 
        });
...
const createphddeclaration=async()=> {
    const b2=await removedirectoryall(phddir);
    const b1=await createdirectoryall(phddir);
    const a1=await createphd();
    const b=await createarchiveall(phddir,"phd_declaration");
   // processfile();
}
