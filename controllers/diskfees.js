...
exports.getaddfees= (req,res) => {
    //res.cookie("user","Akshata");
    const user1=req.cookies['user'];
    //console.log(user1);
    if(user1) {
        res.status(200).render('addfees', {
            title: 'Add fees'
        });
    } else {
        req.flash("error", "You have been logged out. Please login to continue.");
...
exports.getviewfees= async (req,res) => {
    try{
        const user1=req.cookies['user'];
        const colid=req.cookies['colid'];
        if(user1) {
            ...
            .where('colid')
            .equals(colid);
            res.status(200).render('viewfees', {
                categories: lcat1233,
                title: 'List fees config'
...
exports.geteditfees= async (req,res) => {

    try{
        const user1=req.cookies['user'];
        if(user1) {
            const leditcat= await Pat.findById(req.params.id);
            res.status(200).render('editfees', {
                pub: leditcat,
                title: 'Edit fees template'
