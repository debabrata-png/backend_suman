...
const Pat=require('./../Models/examenr');
    ...
    upload(req,res,function(err) {
        ...
        if(user1) {
            ...
            try {
                ...
                res.redirect('/addexamenrol/' + req.params.id);
...
exports.deleteenrollment= async (req,res) => {
    ...
    try{
       ...
       res.redirect('/viewenrollexam/' + req.params.id);
    } catch(err) {
...
exports.getaddenroll= (req,res) => {
    ...
    //console.log(user1);
    if(user1) {
        res.status(200).render('addexamenr', {
...
exports.getenrollist= async (req,res) => {
    try{
        ...
        if(user1) {
            ...
            res.status(200).render('enrollexamlist', {
                categories: lcat1233,
...
exports.updateenr= async (req,res) => {
    ...
    try{
        ...
        req.flash("success", "Data has been updated successfully" );
        res.redirect('/viewenrollexam/'+ req.query.examcode);
    } catch(err) {
        req.flash("error", "Data could not be added successfully. Error " + err );
        res.redirect('/viewenrollexam/'+ req.query.examcode);
...
exports.geteditexamenr= async (req,res) => {

    try{
        const user1=req.cookies['user'];
        if(user1) {
            const leditcat= await Pat.findById(req.params.id);
            res.status(200).render('editexamenr', {
                pub: leditcat,
                title: 'Edit',
...
exports.getmyproctoring= async (req,res) => {
    try{
        ...
        if(user1) {
            ...
            .gte(dt1);
            //console.log(lcat1233);
            res.status(200).render('viewmyproctor', {
                categories: lcat1233,
                title: 'List enrolled students',
                user: user1
...
exports.delallexamenrollment= async (req,res) => {

    try{
        const user1=req.cookies['user'];
        const colid=req.cookies['colid'];
        //await Pat.findByIdAndDelete(req.body.category_id);
        await Pat.deleteMany({ colid: colid, user: user1, examcode: req.body.examcode });
