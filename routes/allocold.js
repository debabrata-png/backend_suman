...
router.get('/available', async (req, res) => {
    const { colid, program, semester, examode, year } = req.query;
try {

const result = await StudAlloc.aggregate([
{ $match: { colid, program, semester, examode, year, $or: [ { faculty: '' }, { faculty: null } ] } },
...
try {
const { course, colid, examode, count } = req.body;
...
{ $match: { course, colid, examode, $or: [ { faculty: '' }, { faculty: null } ] } },
{ $sample: { size: count } },
{ $project: { _id: 1 } }
]);
...
{ _id: { $in: ids } },
{ $set: { faculty: 'F', facultyid: 'f@f.com' } }
);
